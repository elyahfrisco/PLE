<template>
  <jet-modal
    :id="'modal-parameter-registration-price-' + season.id"
    maxWidth="lg"
    :fade="false"
    :title="
      'Paramètre frais d\'INSRIPTION ' +
        establishment.name +
        ',<br> Saison : ' +
        season.year_start +
        ' - ' +
      ...
      <div class="col-md-6">
        ...
        <div class="form-group">
          ...
          <input
            type="number"
            class="form-control"
            v-model="form_child.reduced_price_child"
...
export default {
  ...
  beforeMount() {
    if (this.season.registration_price) {
      for (var price of this.season.registration_price) {
  ...
  methods: {
    ...
    save(form_data, callback = null) {
      this.$inertia.post(
        route("establishments.seasons.prices.registration", {
