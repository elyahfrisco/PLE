              ...
              <template v-if="!loadsManagementFees[user.id]">
                  ...
                  <div class="">
                    ...
                    >
                      ...
                      <button
                        ...
                        @click.prevent="removeManagementFees(user.id)"
                      >
                ...
                <span v-else>
                  <button
                    ...
                    @click.prevent="() => AddManagementFees(user.id)"
                  >
                    <i class="fa fa-euro-sign mr-1"></i> Ajouter
                  </button>
                </span>
              </template>
            </td>
            <td class="text-center">{{ totalCartUser(user.id) }}€</td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    initSession() {
      if (this.$refs.selectUser) this.$refs.selectUser.clear();
    ...
    getSessions() {
      if (this.form.pass_id) {
        ...
        this.last_date_start = this.form.date_start;
        this.last_date_end = this.form.date_end;
        axios
            ...
            if (response.data.data != undefined) {
              ...
              setTimeout(() => {
                if (
                  ...
                  (!this.last_date_end ||
                    this.last_date_start != this.form.sessions[0].date)
                ) {
    ...
    AddManagementFees(user_id) {
      this.form.fees_check[user_id].not_having_paid_management_fees = true;
      this.loadsManagementFees[user_id] = true;
      this.getSeasonManagementFees(user_id);
    },
    removeManagementFees(user_id) {
      this.form.fees_check[user_id].not_having_paid_management_fees = false;
    ...
    totalCartUser(user_id) {
      var total = 0;
      if (
        this.form.fees_check[user_id] &&
        this.form.fees_check[user_id].not_having_paid_registration_fees
      ) {
        switch (this.form.fees_check[user_id].type_registration_fees) {
          case "normal":
            if (
              !isNaN(
                this.form.fees_check[user_id].season_registration_fees.price
              )
            )
              total +=
                this.form.fees_check[user_id].season_registration_fees.price *
      ...
      if (
        ...
        switch (this.form.fees_check[user_id].type_management_fees) {
          case "normal":
            if (
              !isNaN(this.form.fees_check[user_id].season_management_fees.price)
            )
              total +=
                this.form.fees_check[user_id].season_management_fees.price * 1;
            break;
          case "reduced":
            if (
              !isNaN(
      ...
      if (this.form.sessions[0]) {
        if (this.form.pass_type == "one_session") {
          if (!isNaN(this.form.sessions[0].price.price))
            total += this.form.sessions[0].price.price * 1;
        } else if (this.form.pass_type == "trimester") {
          for (let i = 0; i < this.form.sessions.length; i++) {
            if (
              this.form.sessions[i] &&
              this.form.date_end >= this.form.sessions[i].date
            ) {
              if (!isNaN(this.form.sessions[i].price.price))
                total += this.form.sessions[i].price.price * 1;
            } else {
              break;
            }
          }
