                        ...
                        <div
                          ...
                          <p class="text-build-content" style="text-align: center; margin: 10px 0; margin-top: 10px;"
                            data-testid="KtWjoB-VjqyjkXDsekX8O"><span
                              style="color:#0c5d89;font-family:Verdana;font-size:20px;line-height:20px;"><b>Bonjour</b></span> <br>
                              <p>
                                D'apres votre demande de récuperation de votre séance de <span style="text-transform: uppercase">{{ $session_to_catch_up->activity->name }}</span> ( du
                                {{ Carbon::parse($session_to_catch_up->time_start)->format('l d F Y') }}
                                {{ Carbon::parse($session_to_catch_up->time_start)->format('H:i') }} à
                                {{ Carbon::parse($session_to_catch_up->time_end)->format('H:i') }}),
                                <br>
                                nous avons une place disponible pour vous le
                                {{ Carbon::parse($session_for_catch_up->time_start)->format('l d F Y') }},
                                {{ Carbon::parse($session_for_catch_up->time_start)->format('H:i') }} à
                                {{ Carbon::parse($session_for_catch_up->time_end)->format('H:i') }}.
                              </p>
                          </p>
                          ...
                          <div style="margin: 40px 0;">
                            <a target="_blank"
                               ...
                               href="{{ route('recuperation_requests.presence.confirm', $recuperation_request->id) }}"
                               style="padding: 10px; background: #38c172; text-decoration: none; color: white; border-radius: 5px">Cliquer ici
                                pour confirmer votre
                                presence</a>
                            <a target="_blank"
                               class="btn btn-success"
                               href="{{ route('recuperation_requests.cancel', $recuperation_request->id) }}"
                               style="padding: 10px; background: #fb4f38; text-decoration: none; color: white; border-radius: 5px">Annuler la
                                demande de récuperation</a>
                        </div>
